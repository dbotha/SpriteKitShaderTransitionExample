...
void main( void ) {
    ...
    float centreColumn = float(NUM_COLUMNS) / 2.0;
    float centreRow = float(NUM_ROWS) / 2.0;
    
    float dx = float(row) - centreRow;
    float dy = float(column) - centreColumn;
    float distFromCentre = dx * dx + dy * dy;
    
    float maxDx = float(NUM_ROWS) - centreRow;
    float maxDy = float(NUM_COLUMNS) - centreColumn;
    float maxDistFromCentre = maxDx * maxDx + maxDy * maxDy;
    
    float animStartOffset = (u_total_animation_duration - individualTileAnimationDuration) - (distFromCentre / maxDistFromCentre) * (u_total_animation_duration - individualTileAnimationDuration);
